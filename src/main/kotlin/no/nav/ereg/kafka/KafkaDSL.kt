...
open class AKafkaConsumer<K, V>(
    ...
    private fun <K, V> KafkaConsumer<K, V>.pollAndConsumption(
        ...
        runCatching {
            ...
            log.error {
                "Failure during poll - ${it.message}, MsgHost: $currentConsumerMessageHost, " +
                    "Exception class name ${it.javaClass.name}\nMsgBoard: $kafkaConsumerOffsetRangeBoard \nGiven up on poll directly. Do not commit. Do not continue"
            }
                ...
                when (consumerState) {
                    KafkaConsumerStates.IsOk -> {
                        try {
                            ...
                            log.error {
                                "Failure during commit, MsgHost: $currentConsumerMessageHost, " +
                                    "leaving - ${e.message}, Exception name: ${e.javaClass.name}\n" +
                                    "MsgBoard: $kafkaConsumerOffsetRangeBoard"
                            }
