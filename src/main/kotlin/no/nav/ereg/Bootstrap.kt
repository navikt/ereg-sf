...
object Bootstrap {
    ...
    private tailrec fun conditionalSchedule(ev: EnvVar) {
        ...
        if (!ServerState.isOk()) {
            ...
            if (!ShutdownHook.isActive() && Metrics.noOfAttempts.get().toInt() < ev.maxAttempts) waitByState(ev)
        } else {
    ...
    private fun waitByState(ev: EnvVar) {
        val msDelay = when (state) {
            is ServerStates.IsOk -> if (ev.runEachMorning()) getTomorrowMorning(ev) else 1_000
    ...
    private fun getTomorrowMorning(ev: EnvVar) = Duration.between(
        LocalDateTime.now(),
        LocalDateTime.parse(LocalDate.now().plusDays(1).toString() + ev.wakeupTime))
        .toMillis()
}
